...
import { useTheme, Button, Headline, Provider as PaperProvider, TextInput, Text } from 'react-native-paper';
...
export default function App() {
  ...
  const years = Array(20)
    ...
    // More "traditional way to fill array in javascript:"
   /*  const years = Array();
    for (let i = 0; i < 20; i++) {
      const year = i+1;
      years.push({label: years + 'years', value: year.toString()});
    } */
    ...
    const calculate = () => {
     ...
     /*  if ( amount.length === 0 || isNaN(amount) === true) {
      ...
      const divider = Math.pow(1 + interestPerMonth/100, months) * interestPerMonth / 100;
    ...
    <PaperProvider>
      <View style={styles.container}>
        <View style={styles.form}>
          <Headline style={[styles.headline, {color: colors.primary}]}>Annuity loan</Headline>
          <TextInput
            label="Amount"
            mode="outlined"
            style={styles.field}
          ...
          />
          <Text style={[styles.output, {color: colors.primary}]}>{payment.toFixed(2).replace('.' , ',')} €</Text>
